...
def image_to_tensor(img_path, unsqueeze=True):
    ...
    rgb = torch.tensor(np.array(Image.open(img_path)), dtype=torch.float32)
    if unsqueeze:
        rgb = rgb.unsqueeze(0)
...
def disparity_to_tensor(disp_path, unsqueeze=True):
    ...
    # if disp_path.split('/')[-1].split('.')[-1]=='png':
    ...
    disp = torch.tensor(np.array(Image.open(disp_path)), dtype=torch.float32).unsqueeze(0)
    # return disp.float()
    return disp
